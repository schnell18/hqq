...
def patch_model_for_compiled_runtime(
    model, processor, compile=True, warmup=True, patch_accelerate=True
):
    if patch_accelerate:
...
model = Qwen2VLForConditionalGeneration.from_pretrained(
    model_id, cache_dir=cache_dir, torch_dtype=compute_dtype, attn_implementation="sdpa"
).eval()
processor = AutoProcessor.from_pretrained(
    model_id, cache_dir=cache_dir, max_pixels=1280 * 28 * 28
)
...
patch_model_for_compiled_runtime(
    model, processor, warmup=True, patch_accelerate=True, compile=False
)  # compile=True breaks :(
########################################################################
